...
`gower_matcher.py::GowerMatcher` does the same for Gower's `GW_LIFE_TIME` box-office estimates, mapping EVT `film_id` → `gower_id` (`prim_title_no` on the Gower side). Driver: `rematch_gower.py`. Gower data is pulled once via `sql/gower_export.sql`. Both matchers subclass the shared `title_matcher.py::FuzzyTitleMatcher` engine and run **independently** against the EVT catalogue (Gower doesn't chain through Comscore) — see `GOWER.md` for exactly what differs (single title column, `prim_title_no` in place of `title_global_id`, no alt-content flag, multi-snapshot dedup).
...
Matching: NFKD-normalised titles scored with `max(ratio, token_sort_ratio)` (plus `token_set_ratio` for full-title comparisons — see the `allow_subset` note below) across five Comscore title columns (`film_name`/`upper_name`/`title_aka`/`us_title_name`/`short_name`), variant/festival prefix stripping (`GC`/`3D`/`IMAX`/`TFF -`…), a length-ratio guard (rejects short-title impostors like AVATAR→TÁR), a ±1-year release window, and a date-proximity tie-breaker. Each film lands in a confidence tier: **high** (≥0.92 and ≤365 days off), **borderline**, **unmatched**, or **manual** — but a match whose `days_diff` exceeds `MAX_DAYS_HIGH_CONF` (365, and not the "no EVT/source date to compare" `9999` sentinel) is now rejected outright to **unmatched** regardless of score (added in the 2026-09-04 "gower mismatch fix" — a title match this far from the EVT release date is very likely a different release sharing the same title, e.g. FROZEN vs. Frozen 3, THE WOLF MAN vs. Wolf Man); manual overrides are exempt from this check. `_score()`'s `token_set_ratio` term (which gives full credit when one title's words are a subset of the other's) is only applied to full-title comparisons — colon-split title fragments from `_article_variants()` (e.g. "NT LIVE" out of "NT LIVE: VANYA") pass `allow_subset=False` so a deliberately truncated fragment can't score a false 100 against an untruncated source title that happens to start the same way.

After the per-film match loop, `_resolve_duplicate_claims()` (added in the same fix) catches the case where two genuinely different EVT films independently score their best match against the *same* source row (e.g. "PROJECT X" and "PROJECT HAIL MARY" both landing on Gower's "Project Hail Mary") — same-film format/language variants that legitimately share a source ID are left alone (detected via a common `_strip_variant` base title), but a real collision keeps the stronger claim (confidence, then score, then `days_diff`) and re-scores every other claimant against the source pool with that ID removed, iterating (bounded by `max_rounds=5`) since bumping a loser can cascade into a new collision. A film that loses a contested ID is permanently banned from reclaiming it for the rest of the call, so near-duplicate source rows can't make two films bounce between each other indefinitely.

Outputs (under `DATA_DIR/title_matching/comscore/`, mirrored the same way for Gower under `DATA_DIR/title_matching/gower/`):
...
- `comscore_manual_overrides.csv` — **user-authored**: fill `manual_override_cs_id` in the review file, save under this name, and overrides win on the next run (forced to `confidence=manual`, `score=1.0`)

`ComscoreMatcher.re_score()` re-evaluates the whole cache against an updated extract/algorithm without re-pulling (manual overrides preserved) — this is also the fix for a *stale* cached match: since a source table like Gower's `GW_LIFE_TIME` snapshots its own release-date estimate over time, a film matched once and then skipped on every later incremental run (see "Comscore/Gower cache is the checkpoint" below) can end up with a `days_diff`/release-date field that's frozen from whatever the source's release-date estimate was at match time, even after the real source data has since moved on (e.g. a reschedule). `re_score()` re-derives every cached record's date/score fields from the current extract; a plain incremental `build_mapping()` run does not touch already-matched rows at all. Concert films are dropped on both sides (`is_alt_content` in Comscore; `adaptation_type == "concert_film"` from `film_meta_enriched.parquet` on the EVT side).

Since 2026-09-04, `title_matcher.py::FuzzyTitleMatcher._sync_to_s3()` best-effort-mirrors each matcher's cache (+ review file, if one was just written this run) to S3 under `S3_TITLE_MATCHING_PREFIX` (`config.yaml`'s `s3.title_matching_prefix`, currently `"data/film_title_matching"` — a sibling to `S3_PREFIX`'s `"data/meta_data"`, not nested under it), called at the end of every `build_mapping()`/`re_score()` run. `id_bridge.py::build_id_bridge()` mirrors its output the same way. The **local parquet stays authoritative** in all three cases — `title_matcher.py`'s skip-if-already-matched logic reads the local cache back on the next run, not S3 — the S3 copy is purely for other consumers; a transient credentials/network failure here logs a warning and never undoes the local write.
...
### Film meta recheck (added 2026-08-31)

Unlike the other three LLM paths, `refresh.py`'s film_meta path will re-extract a film it has *already* successfully extracted, if that row is missing a prediction-relevant field and still sits inside a release-proximity window where a second web_search look is likely to find something new (e.g. a budget that wasn't public yet at first extraction, months before release, but has since been reported). This is on top of — not instead of — the normal diff-based "not yet in the checkpoint" extraction.

`refresh.py::_films_due_for_recheck()` flags a film for recheck when **all** of:
- **incomplete**: `budget_usd` is null, or fewer than 3 of the top cast slots are filled.
- **in the release window**: `evt_rel_at` within `[-FILM_META_RECHECK_LOOKBACK_MONTHS, +FILM_META_RECHECK_LOOKAHEAD_MONTHS]` of today (currently -2/+6 months — deliberately asymmetric, weighted toward upcoming films, since a first extraction run months before release is the common case that actually benefits from a second look). The null-budget rate holds flat at ~43-49% across every release year 2018-2025 with no downward trend, so rechecking a film outside this window is low-probability spend.
- **a studio film**: `adaptation_type` not in `FILM_META_RECHECK_EXCLUDED_TYPES` (`concert_film`, `documentary`) and `is_concert` is not true — these were never expected to carry a tracked budget/cast. (A genre-text "sports" filter was deliberately left out — too noisy, catches real biopics like *I, Tonya* alongside genuine false positives like *The Karate Kid*; most sports content is filtered upstream by `FILM_META_SKIP_DISTRIBUTORS` anyway.)
- **not rechecked too often or too recently**: `check_count < FILM_META_RECHECK_MAX_CHECKS` (4) and, if `extracted_at` is set, at least `FILM_META_RECHECK_MIN_GAP_DAYS` (45) days have passed since the last extraction.

Rows with no `extracted_at`/`check_count` yet (pre-migration legacy rows, or the first run after this was added) are treated as "never checked" — eligible immediately. `refresh.py::_film_meta_recheck_context()` loads the checkpoint + parquet once and computes the recheck set so `_diff_film_meta` (which lets recheck candidates back through the normal "already done" filter) and `_enrich_film_meta` (which needs each recheck candidate's prior `check_count`, from the same combined view, to increment rather than reset it) agree on the same set. Every extraction — first-time or recheck — writes `check_count` and `extracted_at` (UTC ISO timestamp) onto the row.

`backfill_budget_from_tmdb.py` is a separate, zero-LLM-cost complement to this: a standalone script that fills null `budget_usd` rows in `film_meta_enriched.parquet` from a locally-cached TMDB fuzzy-title-match parquet, but only for `match_confidence == "high"` TMDB matches with a real (>0) budget — never overwrites an existing LLM-sourced figure, and treats a 0/NaN TMDB budget as "TMDB doesn't have it either", not a real zero-dollar budget. Dry-run by default (prints impact only); `--apply` backs up the current parquet first, then writes the backfilled version. Its `TMDB_CACHE_PATH` is a plain hardcoded constant, not a `config.py` path — this cache isn't produced by any matcher/driver script in this repo yet, it's a standalone snapshot (see the Comscore/Gower matchers above for the pattern to follow if it needs to become a proper refreshable path later).
...
- **TMDB is an accepted budget/revenue source in `film_meta_prompts.yaml`** (added 2026-08-31) — the model's first web_search targets Wikipedia/IMDb/TMDB together (TMDB's page carries budget, revenue, cast, and studios as structured fields in one place, checked alongside the others rather than as a fallback); the prompt notes to flag disagreement with another source in the confidence marker. The optional second search (Variety/Deadline/The Numbers/Box Office Mojo) is preferred over TMDB specifically for non-USD productions, since TMDB's budget field is already USD-converted and drops the original local-currency figure that `budget_local`/`budget_currency` need.
...
- **`title_matcher.py::FuzzyTitleMatcher`** — shared fuzzy title-matching engine behind both `comscore_matcher.py::ComscoreMatcher` and `gower_matcher.py::GowerMatcher`: title normalisation/scoring, variant propagation, cross-film duplicate-claim resolution (`_resolve_duplicate_claims`, see "Comscore + Gower matching" above), and cache/review/manual-override I/O (plus a best-effort S3 mirror of both, via `_sync_to_s3`). Subclasses set column names (`ID_COL`/`DATE_COL`/`TITLE_COLS` on the source side, `ID_FIELD`/`TITLE_FIELD`/`DATE_FIELD`/`MATCHED_TITLE_FIELD` on the cache side) and output paths; everything else is identical between sources. See `COMSCORE.md` / `GOWER.md` for the algorithm (note: both docs still say Gower's ID column is `prmry_title_no` — it was renamed to `prim_title_no` on 2026-09-04, see below).
...
- **`id_bridge.py::build_id_bridge()`** — outer-joins `comscore_cache.parquet` + `gower_cache.parquet` on `film_id` into `id_bridge/film_id_bridge.parquet` (local, authoritative), then best-effort mirrors the same file to S3 under `S3_TITLE_MATCHING_PREFIX/id_bridge/`. Pure join, no matching logic.
...
- **`match_thresh` (0.80) vs `HIGH_CONFIDENCE_SCORE` (0.92) are different gates** — anything ≥0.80 **and within `MAX_DAYS_HIGH_CONF` (365) days of the EVT release date** is a match (cached, not retried); only ≥0.92 within that window is labelled `high`, the 0.80–0.92 band is `borderline`. Since 2026-09-04, a match whose `days_diff` exceeds 365 (and isn't the "no date to compare" `9999` sentinel) is forced to `unmatched` regardless of score — score alone is no longer enough to cache a match if the dates are implausibly far apart (see "Comscore + Gower matching" above).
- **A source table's own release-date field can itself be a moving target across snapshots** — confirmed while debugging a Gower mismatch on 2026-09-09: `re_score()` re-derives a cached record from the *current* source extract, but if the underlying source table (e.g. Gower's `CINR_TBM_GW_LIFE_TIME`) legitimately reports a different release-date estimate for a title than EVT's own `rel_at`, `re_score()` will just confirm the same "mismatch" again — it's not a caching bug, and it's not something re-scoring can fix. Before assuming a stale cache, check the source table's raw row(s) for that title directly (not just the code's already-parsed DataFrame) to rule out a genuine source-vs-EVT release-date discrepancy first.
...
- **`rematch_comscore.py` needs `film_lookup.parquet`** — at `DATA_DIR/base_data/film_lookup.parquet` (renamed from `DATA_DIR/look_ups/` on 2026-09-04 — `diagnostics/inspect_comscore_unmatched.py` still hardcodes the old `look_ups` path and will `FileNotFoundError` until updated). The comscore driver joins this for the `film` title column. If it's missing the load will raise `FileNotFoundError`.
...
- **Gower has no `gower_manual_overrides.csv` yet** — the override mechanism (column names driven by each matcher's `ID_FIELD`) is inherited from Comscore's and works the moment that file is created at `DATA_DIR/title_matching/gower/gower_manual_overrides.csv` with a `gower_id` column; no code change needed. Comscore's `comscore_manual_overrides.csv` (with a `cs_id` column) already exists and is in active use.
- **Gower's `ID_COL` is `prim_title_no`** (renamed from `prmry_title_no` on 2026-09-04 — `GOWER.md` still has the old name), not a true global title ID like Comscore's `title_global_id` — it's the closest stable identifier the Gower extract has. Exposed downstream as the cache column `gower_id`.
